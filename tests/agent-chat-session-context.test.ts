...
import { mergeSessionMessages } from "../services/agent/src/message-merge";
...
describe("agent chat session context merge", () => {
  test("appends only the non-overlapping client suffix", () => {
    ...
    expect(mergeSessionMessages(persisted, client)).toEqual([
      user("one"),
      assistant("two"),
      user("three")
    ]);
  ...
  test("handles either side being empty", () => {
    expect(mergeSessionMessages([], [user("client")])).toEqual([
      user("client")
    ]);
    expect(mergeSessionMessages([assistant("persisted")], [])).toEqual([
      assistant("persisted")
    ]);
  });
});
